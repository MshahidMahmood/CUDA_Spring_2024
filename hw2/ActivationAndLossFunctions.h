...
//loss
__device__ float mse(Eigen::MatrixXf& y_true, Eigen::MatrixXf& y_pred)
{
...
__device__ Eigen::MatrixXf mse_prime(Eigen::MatrixXf& y_true, Eigen::MatrixXf& y_pred)
{
	return  2 * (y_pred - y_true) / (y_true.rows()*y_true.cols());
}


__device__ float binary_cross_entropy(Eigen::MatrixXf& y_true, Eigen::MatrixXf& y_pred)
{
	return  (-y_true * y_pred.log()).mean() - (y_true.unaryExpr(one_minus)) * (y_pred.unaryExpr(one_minus)).log());
}

__device__ Eigen::MatrixXf binary_cross_entropy_prime(Eigen::MatrixXf& y_true, Eigen::MatrixXf& y_pred)
{
